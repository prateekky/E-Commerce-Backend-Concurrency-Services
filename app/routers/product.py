...
from app.core.redis import redis_client, CACHE_ALL_PRODUCTS, CACHE_TIME
...
def get_all_products(db: Session = Depends(get_db)):
    ...
    products = (
        db.query(Product)
        .options(joinedload(Product.inventory), joinedload(Product.reviews))
        .all()
    )
        ...
        result.append(
            {
                ...
                "reviews": [
                    {"rating": r.rating, "comment": r.comment} for r in p.reviews
                ],
            }
        )
    # Save the response in Redis for 5 minutes
    try:
        redis_client.setex(cache_key, CACHE_TIME, json.dumps(result, default=str))
    except Exception as e:
...
def create_product(
    ...
    try:
        ...
        new_inventory = Inventory(
            product_id=new_product.id, stock=product_in.initial_stock
        )
...
def update_product(
    ...
    return {"message": "Product updated successfully", "product": product_exist}
...
def create_review(
    ...
    existing_review = (
        db.query(Review)
        .filter(Review.product_id == product_id, Review.user_id == curr_user.id)
        .first()
    )
